...
| **Aggregates from children** — a parent's `Rating` as the average of its reviews' | **Not built.** The marketplace computes it by hand in `Product#recompute_rating!` and the controller must remember to call it (friction log, ECS-22). | Once a second aggregate appears; a `counter_cache`-shaped declaration on `has_many` is the likely shape. |
