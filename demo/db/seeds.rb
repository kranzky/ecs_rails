...
# Rich demo data exercising every gem feature. Idempotent-ish: run on a fresh DB.
...
# groups + memberships (join entities)
rubyists = Group.create!; rubyists.name.first = "Rubyists"; rubyists.description.text = "People who enjoy writing Ruby."; rubyists.save!
pioneers = Group.create!; pioneers.name.first = "Computing Pioneers"; pioneers.description.text = "The people who got us here."; pioneers.save!
...
[[ada, rubyists, "owner"], [grace, rubyists, "member"], [alan, rubyists, "member"],
 [ada, pioneers, "member"], [grace, pioneers, "member"], [katherine, pioneers, "member"]].each do |u, g, role|
  m = Membership.create!; m.user = u; m.group = g; m.role.name = role; m.save!
end
